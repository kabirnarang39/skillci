...
// file and checks the warning actually reaches regress's own output, not
// just that regress.ScanStaleGeneratedCases behaves correctly in
// isolation.
func TestRegressCommandWarnsOnStaleGeneratedCase(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		resp := map[string]any{
	...
	staleContent := "# generated by skillci's self-growing eval loop — informational, not part of the case spec\n" +
		"# model: claude-sonnet-5\n" +
		"# detected_at: " + oldTimestamp + "\n" +
		"# actual_response:\n#   (empty)\n" +
		"name: stale-generated-case\nprompt: p\n"
	if err := os.WriteFile(filepath.Join(generatedDir, "stale-generated-case.yaml"), []byte(staleContent), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(out.String(), "1 unaddressed generated eval case(s) older than 14 days") {
		t.Errorf("output = %q, want a staleness warning", out.String())
	}
	if !strings.Contains(out.String(), "stale-generated-case.yaml") {
		t.Errorf("output = %q, want it to name the stale file", out.String())
	}
}
